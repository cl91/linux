...
int pci_msix_vec_count(struct pci_dev *dev)
{
	...
	return list_count_nodes(&bus_data->irq_list);
}
				   ...
				   struct irq_affinity *affd)
{
	...
	if (pci_dev_msi_enabled(dev)) {
		struct lnxdrv_pci_bus_data *bus_data = pdev_to_bus_data(dev);
		BUG_ON(!bus_data);
		unsigned int nvecs = list_count_nodes(&bus_data->irq_list);
		if (nvecs < min_vecs) {
			return -ENOSPC;
		}
...
{
	/* TODO: Call the PCI bus driver to query atomic ops. This is used heavily
	 ...
	 * finally turn it on for the device itself. On most amd64 systems the firmware
	 * will likely have programed this already, so for now we return success. In
	 * the future we will need to have the PCI bus driver probe and enable atomic
	 * ops during enumeration and have the PCI device driver query the bus driver
